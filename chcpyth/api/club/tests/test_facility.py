...
class FacilityTestCase(ClubTestBase):
    """ This is the unit test case for Facility model.
    """
    def __init__(self, *args, **kwargs):
        super(FacilityTestCase, self).__init__(*args, **kwargs)

        data_prov = FacilityDataProvider()
        self.entity1 = data_prov.get_entity1()
        self.entity2 = data_prov.get_entity2()

    def test_CRUD(self):
        """ Test CRUD functions of the Facility model.
        """
        ...
        savedEntity = Facility.objects.get(pk=entity1.id)
        self.assertIsNotNone(savedEntity)
        self.assertIsNotNone(savedEntity.ref)

        entity1.name = "Awesome facility"
        entity1.save()
        updatedEntity = Facility(pk=entity1.id)
        self.assertEqual(entity1.name, "Awesome facility")

        entity1.delete()
        with self.assertRaises(Facility.DoesNotExist):
            Facility.objects.get(pk=entity1.id)
